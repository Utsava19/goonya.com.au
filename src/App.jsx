...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // ── HERO: use fromTo so start AND end are explicit — no ambiguity ──
    const tl = gsap.timeline({ delay:.15 });
    tl.fromTo(q1(".home-kicker"),
        { opacity:0, y:14 }, { opacity:1, y:0, duration:.5, ease:"power2.out" })
      .fromTo(q(".hero-line"),
        { opacity:0, y:36 }, { opacity:1, y:0, stagger:.09, duration:.75, ease:"power3.out" }, "-=.25")
      .fromTo(q1(".hero-sub"),
        { opacity:0, y:14 }, { opacity:1, y:0, duration:.55, ease:"power2.out" }, "-=.35")
      .fromTo(q1(".hero-ctas"),
        { opacity:0, y:10 }, { opacity:1, y:0, duration:.5,  ease:"power2.out" }, "-=.3")
      .fromTo(q1(".hero-right"),
        { opacity:0, x:36 }, { opacity:1, x:0, duration:.9,  ease:"power3.out" }, "-=.65");

    // ── MARQUEE ──
    const marquee = q1(".marquee-track");
    if (marquee) gsap.to(marquee, { xPercent:-50, duration:26, repeat:-1, ease:"none" });

    // ── SCROLL REVEALS — fromTo so always correct ──
    ...
    if (dbars.length && dash) {
      ...
      ScrollTrigger.create({
        ...
        onEnter:() => gsap.to(dbars, {
          scaleY:1, duration:1, ease:"power3.out", stagger:.05,
